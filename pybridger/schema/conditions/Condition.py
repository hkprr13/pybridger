...
class Condition:
    ...
    #---------------------------------------------------------------------------
    @public
    def toQuery(self, placeHolder = "?"):
        # leftがConditonオブジェクトなら,再帰的にtoQuery()を呼び出す
        if isinstance(self.left, Condition):
            # 複数条件をネストしてSQLに変換するため
            leftSql, leftValues = self.left.toQuery(placeHolder)
        # leftが単なる列名の場合文字列化してSQLとして扱う
        else:
            leftSql    = str(self.left)
            leftValues = []
        if self.right is not None \
        and hasattr(self.right, "toQuery"):
            rightSql, rightValues = self.right.toQuery()
            if self.tableName:
        ...
        # 論理式(AND/OR/NOT)
        # rightがconditionオブジェクトなら, 再帰的にtoQuery()を呼び出す
        if isinstance(self.right, Condition):
            rightSql, rightValues = self.right.toQuery(placeHolder)
            sql = f"{leftSql} {self.operator} {rightSql}"
